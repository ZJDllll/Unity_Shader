...
float GetShadowMaskAttenuation(float2 uv) {
    ...
    #if defined(SHADOWS_SHADMASK)
        float4 mask = tex2D(_CameraGBfferTexture4, uv);
...
fixed4 FragmentProgram (Interpolators i) : SV_Target
{
    ...
    float3 rayToFarPanel = i.ray * _ProjectionParams.z / i.ray.z;
    float3 viewPos = rayToFarPanel * depth;
    ...
    float4 color = UNITY_BRDF_PBS(
            ...
            normal,viewDir,CreateLight(uv,worldPos,viewPos.z),indirectLight);
