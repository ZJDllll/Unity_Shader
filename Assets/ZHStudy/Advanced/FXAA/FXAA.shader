Shader "Hidden/FXAA"
{
    ...
    SubShader
    {
        ...
        struct LuminanceData{
            float m,n,e,s,w;
            float ne,nw,se,sw;
            float heighest,lowest,contrast;
        };
        LuminanceData SampleLuminanceNeighborhood(float2 uv){
            ...
            l.heighest = max(max(max(max(l.n,l.e),l.s),l.w),l.m);
            l.lowest = min(min(min(min(l.n,l.e),l.s),l.w),l.m);
            l.contrast = l.heighest - l.lowest;
            
            return l;
        }

        bool ShouldSkipPixel(LuminanceData l){
            float threshold = max(_ContrastThreshold,_RelativeThreshold * l.heighest);
            return l.contrast < threshold;
        }
        
        float DeterminePixelBlendFactor(LuminanceData l){
            float filter = 2*(l.n+l.w+l.s+l.e);
        ...
        EdgeData DetermineEdge(LuminanceData l){
            EdgeData e;
            float horizontal = 2*abs(l.n+l.s - 2*l.m) + abs(l.nw+l.sw - 2*l.w)+abs(l.ne+l.se - 2*l.e);
            float vertical = 2*abs(l.w+l.e - 2*l.m) + abs(l.ne+l.nw - 2*l.n)+abs(l.se+l.sw - 2*l.s);
        ...
        float DetermineEdgeBlendFactor(LuminanceData l,EdgeData e,float2 uv){
            ...
            //边缘最后一个点是凸 并且当前像素点是在更亮的这边，此像素点不需要混合
            if(deltaSign == (l.m - edgeLuminance >=0)){
                return 0;
            }
            //当前像素点在正向与负向距离相加的中间的话不需要混合，越靠近远端混合值越大
            return 0.5 - shortestDistance/(pDistance + nDistance);
        }
        float4 ApplyFXAA(float2 uv){
            LuminanceData l =SampleLuminanceNeighborhood(uv);
            // if(l.contrast < _ContrastThreshold){
            //     float4 res = float4(1,0,0,0);
            //     if(l.contrast < _RelativeThreshold * l.heighest){
            //         res += float4(0,1,0,0);
            //     }
            //     return res;
            // }
